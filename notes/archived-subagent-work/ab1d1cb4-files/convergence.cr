...
def walk_shared(
  ...
  if depth >= 1
    # log-space mean: log((exp(lpa) + exp(lpb)) / 2)
  ...
  node_a.children.each do |token, child_a_idx|
    ...
    # Compute log P(token | node_a) and log P(token | node_b)
    ...
    # pi_min tracks the minimum single-edge probability across the path
...
STDERR.puts "[convergence] Trie A built in #{(Time.monotonic - t0).total_seconds.round(2)}s, #{trie_a.nodes.size} nodes"

STDERR.puts "[convergence] Building trie B (depth=#{max_depth})..."
t0 = Time.monotonic
trie_b = Trie.new
trie_b.build(tokens_b, max_depth)
STDERR.puts "[convergence] Trie B built in #{(Time.monotonic - t0).total_seconds.round(2)}s, #{trie_b.nodes.size} nodes"
...
walk_shared(
  ...
  0, 0,
  0, max_depth,
  0.0, 0.0,  # log_pi starts at 0 (product = 1 before any edge)
  Float64::INFINITY,  # log_pi_min: will be set on first edge
  results,
  min_count
)

STDERR.puts "[convergence] Walk complete in #{(Time.monotonic - t0).total_seconds.round(2)}s, #{results.size} shared paths"
...
File.open(csv_path, "w") do |f|
  f.puts "depth,log_pi_a,log_pi_b,log_pi_mean,log_abs_err,count_a,count_b,log_pi_min"
  results.each do |r|
    # Handle the initial infinity for paths of depth 1 (log_pi_min = first edge min)
    lpm = r.log_pi_min.infinite? ? 0.0 : r.log_pi_min
    f.puts "#{r.depth},#{r.log_pi_a},#{r.log_pi_b},#{r.log_pi_mean},#{r.log_abs_err},#{r.count_a},#{r.count_b},#{lpm}"
  end
end

...
This experiment's **purpose is still valid**, but the restored runs in this
directory were taken on the pre-fix AGPT trainer and should be treated as
provisional notes, not current evidence.

Why:

- The AGPT side predates commit `1c858c0` (2026-04-23), which fixed the frozen
  `Wk` / `Wv` / bias bug in `bin/agpt_train`.
- The old SGD side also hit a separate cuBLAS / NaN issue.

This is why the directory was removed from `main` in commit `15323ef`
("Remove pre-fix sgd-sanity-check experiment") and later restored only as a
cleanup/reassessment reference.

For now, treat the restored logs here as branch artifacts to learn from while
planning the redo. Do not cite their quantitative results without re-running
the experiment on the post-fix trainer / stable SGD baseline.
...
## Results from the pre-fix run set

The table below is preserved only as a record of what the old branch claimed.
It is **not** yet a trusted current conclusion.
...
Historical implication only: the branch was leaning toward
`--mass-weight linear`, but that conclusion needs to be re-earned on the
...
When this experiment is revived after cleanup, keep the original question but
restart from scratch:

1. Use post-fix `bin/agpt_train` only.
2. Use a known-good SGD reference path (likely openblas, not the old cuBLAS run).
3. Keep the comparison focused on:
   - AGPT aggregation correctness
   - weighting choice (`off`, `log`, `sqrt`, `linear`)
4. Write new results into a fresh summary instead of mixing them with the
   invalidated logs restored from branch history.

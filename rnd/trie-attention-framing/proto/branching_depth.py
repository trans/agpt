"""For each prefix-tree leaf, compute d* = depth at which its prefix first
becomes unique (mass first drops to 1) walking root→leaf. Histogram d*
across all mass-1 leaves and report mean + median.

Confirms the framing's first descriptive prediction:
    d* ≈ log₂(N) / per-char-entropy

Usage:
    python3 branching_depth.py [TRIE_DIR]

If TRIE_DIR is omitted, defaults to gutenberg_5m_d32 for backward compat
with the original p2s-attention version.
"""
import os, struct, sys, time
from collections import Counter

DEFAULT_PREFIX_DIR = "/home/trans/agpt-tries/gutenberg_5m_d32_radix_corpus"
...
def load_full(dirpath):
    ...
    for d in range(depth_file_count):
        ...
        struct.unpack_from("<i", buf, pos); pos += 4  # stored_d
...
def main():
    prefix_dir = sys.argv[1] if len(sys.argv) > 1 else DEFAULT_PREFIX_DIR
    parent_arr, fcd_arr, _, mass_arr, n_records = load_full(prefix_dir)

    print("Identifying leaves...")
    ...
    print("Computing d* per mass-1 leaf...")
    t0 = time.time()
    d_star_hist = Counter()
    mass1_leaf_count = 0
    ...
    print(f"  no-unique:                  {no_unique_count}")

    if not d_star_hist:
        print("  no mass-1 leaves found — corpus too small for d=cap?")
        return

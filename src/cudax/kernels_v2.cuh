...
__global__ static void agpt_loss_per_query_kernel_v2(
    ...
    if (tid == 0) {
        ...
        if (is_endpoint) {
            ...
            float grad_scale = query_grad_scales ? query_grad_scales[q] : 1.0f;
            if (grad_scale != 1.0f) {
                for (int j = 0; j < V; j++) grad_row[j] *= grad_scale;
            }
            loss_out[q] = loss;
        } else {
...
static inline void launch_agpt_loss_per_query_v2(const float* logits,
    ...
    agpt_loss_per_query_kernel_v2<<<T_q, threads, smem>>>(
        ...
        query_weights, query_grad_scales, d_logits, loss_out, T_q, V);
}

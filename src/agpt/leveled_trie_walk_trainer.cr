      ...
      def train_epoch(model : MiniGPT) : {Float64, Int32}
        ...
        @reader.depth_file_count.times do |depth|
          ...
          results, this_caches = BatchedDepthForward.forward_depth(
            eligible, node_ancestor_ids, node_positions, kv_store, model, @reader, prev_caches
          )
          ...
          subtries.each do |_root_id, subtrie_results|
            ...
            BatchedDepthBackward.backward_depth(
              subtrie_results, subtrie_grads, grad_accums, kv_store, model, @reader, this_caches
            )
